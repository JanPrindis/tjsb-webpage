document.addEventListener('DOMContentLoaded', () => {
    ...
    const itemsHtml = order.items.map(item => `
        <li class="summary-item">
            <span class="sum-name">${item.quantity}x ${item.name} ${item.size ? `<small>(Vel: ${item.size})</small>` : ''}</span>
    ...
    container.innerHTML = `
        <div class="success-header">
            ...
            <h2 style="color: var(--yellow, #ffd700); margin-bottom: 0.5rem;">Děkujeme za rezervaci, ${order.customerName.split(' ')[0]}!</h2>
        ...
        <div class="summary-section">
            ...
            <p style="color: rgba(255,255,255,0.8); margin: 0;">
                Jméno: ${order.customerName}<br>
                E-mail: ${order.customerEmail}<br>
